...
function PropertyDetails() {
  ...
  useEffect(() => {
    (
      async () => {
        const response = await fetch(`http://localhost:8002/api/properties/${id}`)
  
        const property = await response.json();
  
        setProperty({ ...property, name: property.name });
        setProperty({ ...property, room: property.number_of_rooms });
        setProperty({ ...property, bathroom: property.number_of_bathrooms });
        setProperty({ ...property, description: property.description });
        setProperty({ ...property, address: property.address });
        setProperty({ ...property, city: property.city });
        setProperty({ ...property, state: property.state });
        setProperty({ ...property, pincode: property.pincode });
        setProperty({ ...property, image: property.image });
        setProperty({ ...property, name: property.name });
      }
    )();
  }, [])
